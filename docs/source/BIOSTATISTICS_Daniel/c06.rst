...
https://sphinxcontrib-text-styles.readthedocs.io/en/stable/
...
After studying this chapter, the student will
   ...
   :text-blue:`Comprender la importancia y los principios básicos de la estimación.`

   2. be able to calculate interval estimates for a variety of parameters.

   :text-blue:`Ser capaz de calcular estimaciones de intervalo para una variedad de parámetros.`

   3. be able to interpret a confidence interval from both a practical and a probabilistic viewpoint.

   :text-blue:`Ser capaz de interpretar un intervalo de confianza desde un punto de vista tanto práctico como probabilístico.`

   4. understand the basic properties and uses of the t distribution, chi-square distribution, and F distribution.

   :text-blue:`comprender las propiedades básicas y los usos de la distribución t, la distribución chi-cuadrado y la distribución F.`
...
:text-blue:`Pasamos ahora a considerar la estimación, la primera de las dos áreas generales de la inferencia estadística. La segunda área general, la prueba de hipótesis, se examina en el próximo capítulo. En el capítulo 1 aprendimos que la estadística inferencial se define de la siguiente manera.`
...
:text-blue:`La inferencia estadística es el procedimiento mediante el cual llegamos a una conclusión sobre una población a partir de la información contenida en una muestra extraída de esa población.`
...
:text-blue:`El proceso de estimación implica calcular, a partir de los datos de una muestra, algún estadístico que se ofrece como una aproximación del parámetro correspondiente de la población de la que se extrajo la muestra.`
...
:text-blue:`Estos dos ejemplos han implicado un interés en estimar, respectivamente, una media poblacional y una proporción poblacional. Otros parámetros, cuya estimación cubriremos en este capítulo, son la diferencia entre dos medias, la diferencia entre dos proporciones, la varianza poblacional y la relación de dos varianzas.`
...
:text-blue:`Encontraremos que para cada uno de los parámetros que analizamos, podemos calcular dos tipos de estimación: una estimación puntual y una estimación de intervalo.`

**DEFINITION**

A point estimate is a single numerical value used to estimate the corresponding population parameter.

:text-blue:`Una estimación puntual es un valor numérico único que se utiliza para estimar el parámetro poblacional correspondiente.`
...
:text-blue:`Una estimación de intervalo consta de dos valores numéricos que definen un rango de valores que, con un grado específico de confianza, probablemente incluya el parámetro que se está estimando.`


These concepts will be elaborated on in the succeeding sections.

:text-blue:`Estos conceptos se desarrollarán en las secciones siguientes.`
...
:text-blue:`Tenga en cuenta que a un único valor calculado se le denomina estimación. La regla que nos dice cómo calcular este valor, o estimación, se denomina estimador. Los estimadores suelen presentarse como fórmulas. Por ejemplo,`
...
:text-blue:`es un estimador de la media poblacional,` :math:`\mu`. text-blue:`El único valor numérico que resulta de evaluar esta fórmula se llama estimación del parámetro` :math:`\mu`.
...
text-blue:`En muchos casos, un parámetro puede ser estimado por más de un estimador. Por ejemplo, podríamos usar la mediana muestral para estimar la media poblacional. ¿Cómo decidimos entonces qué estimador utilizar para estimar un parámetro determinado? La decisión se basa en una medida objetiva o un conjunto de criterios que reflejan alguna propiedad deseada de un estimador particular. Cuando se comparan con estos criterios, algunos estimadores son mejores que otros. Uno de estos criterios es la propiedad de imparcialidad.`

**DEFINITION**

An estimator, say, T, of the parameter :math:`\theta` is said to be an unbiased estimator of teta if E(T) = teta
...
text-blue:`E(T) se lee, “el valor esperado de T”. Para una población finita, E(T) se obtiene tomando el valor promedio de T calculado a partir de todas las muestras posibles de un tamaño determinado que puedan extraerse de la población. Esto es :math:`E(T) = \mu_T`. Para una población infinita, E(T) se define en términos de cálculo.`
...
text-blue:`El investigador de salud que utiliza procedimientos de inferencia estadística debe ser consciente de la diferencia entre dos tipos de población: la población muestreada y la población objetivo.`
...
text-blue:`La población muestreada es la población de la que realmente se extrae una muestra.`

**DEFINITION**

The target population is the population about which one wishes to make an inference.

text-blue:`La población objetivo es la población sobre la cual se desea hacer una inferencia.`
